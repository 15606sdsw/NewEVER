...
// ファイルを文字列として読み込む関数
// 3DGRTのoptixTracer.cppから移植
void readSourceFile(std::string& str, const std::string& filename) {
...
Forward::Forward(const OptixDeviceContext &context, int8_t device,
  ...
  OptixModule module2 = nullptr;
  {
    ...
    std::string input;
    if (enable_backward) {
      input = Forward::load_ptx_data();
    } else {
      input = Forward::load_fast_ptx_data();
    }
    OPTIX_CHECK_LOG(optixModuleCreateFromPTX(
        context, &module_compile_options, &pipeline_compile_options,
        input.c_str(), input.size(), log, &sizeof_log, &module));

    // input = Forward::load_ptx_data2();
    
    /*
    std::string ptx_code;
        // enable_backwardフラグに応じて、事前コンパイル済みのPTXファイルを読み込む
        if (enable_backward) {
            // CMakeLists.txtで shaders.slang から shaders.ptx が生成される
            readSourceFile(ptx_code, "shaders.ptx");
        } else {
            // CMakeLists.txtで fast_shaders.slang から fast_shaders.ptx が生成される
            readSourceFile(ptx_code, "fast_shaders.ptx");
        }
