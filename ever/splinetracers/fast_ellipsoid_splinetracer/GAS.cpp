...
void GAS::build(const Primitives &model) {
    ...
    CUdeviceptr d_vertex_buffers[] = { (CUdeviceptr)model.d_vertices };
    OptixBuildInput triangle_input = {};

    triangle_input.type = OPTIX_BUILD_INPUT_TYPE_TRIANGLES;
    // 頂点バッファの情報
    triangle_input.triangleArray.vertexFormat = OPTIX_VERTEX_FORMAT_FLOAT3;
    triangle_input.triangleArray.vertexBuffers = d_vertex_buffers;
    triangle_input.triangleArray.numVertices = model.num_vertices_per_prim * model.num_prims;
    triangle_input.triangleArray.vertexStrideInBytes = sizeof(glm::vec3);    

    // インデックスバッファの情報
    triangle_input.triangleArray.indexFormat = OPTIX_INDICES_FORMAT_UNSIGNED_INT3;
    triangle_input.triangleArray.indexBuffer = (CUdeviceptr)model.d_indices;
    triangle_input.triangleArray.numIndexTriplets = model.num_triangles_per_prim * model.num_prims;
    ...
    OPTIX_CHECK( optixAccelBuild(
                ...
                &gas_handle,
                &property,          // emitted property list for compaction
