...
public:
  ...
  void add_primitives(const torch::Tensor &means, const torch::Tensor &scales,
    ...
    TORCH_CHECK(opacities.size(0) == numPrimitives,
    ...
    model.densities = reinterpret_cast<float *>(opacities.data_ptr());
    model.features = reinterpret_cast<float *>(colors.data_ptr());
    model.num_prims = numPrimitives;
    model.prev_alloc_size = NUM_AABBS;
    model.aabbs = D_AABBS;
    // create_aabbs(model);
    create_icosahedron_primitives(model, reinterpret_cast<float *>(opacities.data_ptr()));
    D_AABBS = model.aabbs;
...
public:
  ...
  fesPyForward(const fesOptixContext &context, const torch::Device &device,
      ...
      : device(device),
        forward(context.context, device.index(), model.model, enable_backward, false),
