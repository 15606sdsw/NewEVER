...
class SplineTracer(Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(
            mean, scale, quat, opasity, color, rayo, rayd, tri_collection, wcts, out['initial_drgb'], initial_inds, half_attribs
        )
    ...
    def backward(ctx, grad_output: torch.Tensor, return_extras=False):
        ...
        dL_dopasities = torch.zeros((num_prims), dtype=torch.float32, device=device)
        ...
        return (
            ...
            dL_dopasities.clip(min=-50, max=50).reshape(opasity.shape),
